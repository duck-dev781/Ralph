...
bool applySDManifest(String url){
  WiFiClientSecure c; c.setInsecure(); HTTPClient h;
  if(!h.begin(c,url)||h.GET()!=200){h.end();return false;}
  String body=h.getString();h.end();
  int p=0;bool any=false;
  while(p<body.length()){
    int e=body.indexOf("\n",p);if(e<0)e=body.length();
    String line=body.substring(p,e);line.trim();p=e+1;
    if(!line.length()||line[0]=='#')continue;
    int sep=line.indexOf("|");if(sep<2)continue;
    String target=safePath(line.substring(0,sep)),url2=line.substring(sep+1);url2.trim();
    WiFiClientSecure fc;fc.setInsecure();HTTPClient f;
    if(f.begin(fc,url2)&&f.GET()==200){
      SD_MMC.mkdir(target.substring(0,target.lastIndexOf("/")));
      File out=SD_MMC.open(target,FILE_WRITE);
      if(out){WiFiClient *s=f.getStreamPtr();uint8_t b[1024];int remaining=f.getSize();while(remaining!=0){int want=sizeof(b);if(remaining>0&&remaining<want)want=remaining;int n=s->readBytes(b,want);if(n<=0)break;out.write(b,n);if(remaining>0)remaining-=n;}out.close();any=true;}
    }f.end();
  }
  return any;
}
void checkGitHubUpdate(){if(!wifiOK)return;WiFiClientSecure client;client.setInsecure();HTTPClient h;if(!h.begin(client,UPDATE_MANIFEST))return;int code=h.GET();if(code!=200){h.end();return;}String body=h.getString(),ver,fw,sdm;parseManifest(body,"VERSION",ver);parseManifest(body,"FIRMWARE",fw);parseManifest(body,"SD_MANIFEST",sdm);h.end();if(ver.length()&&versionNumber(ver)>versionNumber(cfg.installedVersion)){if(sdm.length())applySDManifest(sdm);if(fw.length()){WiFiClientSecure fc;fc.setInsecure();HTTPClient f;if(f.begin(fc,fw)&&f.GET()==200){int len=f.getSize();if(Update.begin(len>0?len:UPDATE_SIZE_UNKNOWN)){Update.writeStream(f.getStream());if(Update.end(true)){cfg.installedVersion=ver;save();delay(500);ESP.restart();}}}f.end();}}}
