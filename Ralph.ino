...
uint32_t lastUpdateCheck=0,lastWiFiAttempt=0;
...
bool sdOK=false, mpuOK=false, bleConnected=false, brakeOn=true;
...
struct Settings {
  ...
  String updateChannel="stable", firmwareVersion="1.0.0", sdVersion="1.0.0";
...
void setBrake(bool on){brakeOn=on;if(BRAKE_LED_PIN>=0)digitalWrite(BRAKE_LED_PIN,on);}
...
void mpu(){if(!mpuOK)return;int16_t x,y,z;if(!mraw(x,y,z))return;ax=x/16384.f;ay=y/16384.f;az=z/16384.f;float mag=sqrtf(ax*ax+ay*ay+az*az),d=fabsf(mag-lastMag);lastMag=mag;shake*=.88f;if(d>.75f)shake+=d;if(d>WAKE_DELTA||fabsf(mag-1)>.1f){lastMove=millis();if(state==SLEEPING){state=WAKING;stateUntil=millis()+1800;setBrake(true);}}if(state!=SLEEPING&&state!=SETUP_MODE){bool upside=az<-UPSIDE||ay<-UPSIDE||ax<-UPSIDE;if(upside&&mag>.75&&mag<1.25){state=FALLEN;stateUntil=millis()+3000;}else if(shake>HARD_SHAKE){state=DIZZY;stateUntil=millis()+5000;}}}
...
if(c=="SLEEP"){state=SLEEPING;setBrake(false);reply("Goodnight.");return;}if(c=="WAKE"){state=WAKING;stateUntil=millis()+1800;setBrake(true);reply("I'm awake!");return;}
if(c=="STATUS"){reply("tempF="+String(tempF,1)+" mpu="+String(mpuOK?"ok":"missing")+" sd="+String(sdOK?"ok":"missing")+" wifi="+String(wifiOK?"ok":"off")+" fw="+cfg.firmwareVersion+" sdv="+cfg.sdVersion+" state="+String((int)state));return;}
if(c=="RESETSETUP"){cfg.complete=false;save();state=SETUP_MODE;setBrake(false);reply("SETUP RESET");return;}
...
void apiStatus(){if(!webAuth())return;String j="{\"name\":\""+jsonEscape(cfg.name)+"\",\"deviceId\":\""+cfg.deviceId+"\",\"registered\":"+String(cfg.registered?"true":"false")+",\"wifi\":"+String(wifiOK?"true":"false")+",\"ssid\":\""+jsonEscape(cfg.wifiSSID)+"\",\"ip\":\""+(wifiOK?WiFi.localIP().toString():"")+"\",\"firmwareVersion\":\""+cfg.firmwareVersion+"\",\"sdVersion\":\""+cfg.sdVersion+"\",\"channel\":\""+cfg.updateChannel+"\",\"sd\":"+String(sdOK?"true":"false")+"}";web.send(200,"application/json",j);}
void apiFiles(){if(!webAuth())return;String p=safePath(web.hasArg("path")?web.arg("path"):"/");File dir=SD_MMC.open(p);if(!dir||!dir.isDirectory()){web.send(404,"text/plain","Not a directory");return;}String j="[";File f=dir.openNextFile();bool first=true;while(f){if(!first)j+=",";String full=String(f.name());String base=full;int slash=base.lastIndexOf("/");if(slash>=0)base=base.substring(slash+1);j+="{\"name\":\""+jsonEscape(base)+"\",\"path\":\""+jsonEscape(full)+"\",\"size\":"+String((unsigned long)f.size())+",\"dir\":"+String(f.isDirectory()?"true":"false")+"}";first=false;f=f.openNextFile();}j+="]";web.send(200,"application/json",j);}
...
int compareVersion(String a,String b){a.trim();b.trim();if(a.startsWith("v"))a=a.substring(1);if(b.startsWith("v"))b=b.substring(1);int ai[4]={0,0,0,0},bi[4]={0,0,0,0};for(int i=0;i<4;i++){int p=a.indexOf('.');String n=p<0?a:a.substring(0,p);ai[i]=n.toInt();if(p<0)a="";else a=a.substring(p+1);p=b.indexOf('.');n=p<0?b:b.substring(0,p);bi[i]=n.toInt();if(p<0)b="";else b=b.substring(p+1);}for(int i=0;i<4;i++)if(ai[i]!=bi[i])return ai[i]>bi[i]?1:-1;return 0;}
...
void drawPixelAnimation(uint8_t id,uint8_t f){
  ...
  if(id==73)rx=8;
  if(id==74||id==75)rx=6;
  if(id==76)rx=5;if(id==77)rx=4;if(id==78)rx=5;if(id==79)rx=6;
